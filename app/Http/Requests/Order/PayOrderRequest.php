...
class PayOrderRequest extends StoreRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'payment_digitalwallet' => ['required_if:payment_method,BILLETERA DIGITAL', 'in:YAPE'],
    ...
    public function prepareForValidation()
    {
        ...
        foreach ($data['products'] as $index => $item) {
            ...
            $api360Service->update_stock_consultando_360($data_stock);
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $api360 = app(Api360Service::class);
            ...
            foreach ($this->input('products') as $index => $item) {
                ...
                $detail = $api360->update_stock_consultando_360($stockData);
                ...
                $productName = optional(Product::firstWhere('server_id', $stockData['product_id']))->name ?? 'Producto';
                $colorName = optional(Color::firstWhere('server_id', $stockData['color_id']))->name ?? 'Color';
                $sizeName = optional(Size::firstWhere('server_id', $stockData['size_id']))->name ?? 'Talla';

                $validator->errors()->add(
                    "products.$index.quantity",
                    "No hay suficiente stock para {$productName}, {$colorName}, {$sizeName}. Stock disponible: {$detail->stock}, solicitado: {$quantity}."
                );
            }
        });
    }
